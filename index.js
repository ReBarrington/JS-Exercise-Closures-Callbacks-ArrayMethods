...
function processSum(numberList, callback) {
  let sum = numberList.reduce(function(accumulator, num){
...
function processDuplicateFree(list, callback) {
  ...
  return callback(Array.from(set));
}
...
function firstNamesAllCaps(runners) {
  ...
  runners.forEach(function (runner) {
    newArray.push(runner.first_name.toUpperCase())
  })
  return newArray;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const newArr = runners.filter(function(runner) {
...
function tallyUpDonations(runners) {
  const donations = runners.reduce(function(accumulator, item) {
